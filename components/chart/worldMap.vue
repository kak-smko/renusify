...
export default {
  ...
  computed: {
    ...
    paths() {
      let mapPaths = Object.assign({}, this.mapPaths);

      if (!this.Western_Sahara_is_country) {
  ...
  methods: {
    getColor(ratio) {
      ...
      if (/#/.test(this.maxColor)) {
        ...
        color1.g = parseInt(c.substring(2, 4), 16);
        color1.b = parseInt(c.substring(4, 6), 16);
      } else if (/rgb/.test(this.maxColor)) {
        const c = (/rgba?\((.*?)\)/.exec(this.maxColor) || [
          "",
          "0,0,0,1",
        ])[1].split(",");
        color1.r = Number(c[0]) || 0;
        color1.g = Number(c[1]) || 0;
        color1.b = Number(c[2]) || 0;
      }
      if (/#/.test(this.minColor)) {
        const c = this.minColor.slice(-6);
        color2.r = parseInt(c.substring(0, 2), 16);
        color2.g = parseInt(c.substring(2, 4), 16);
        color2.b = parseInt(c.substring(4, 6), 16);
      } else if (/rgb/.test(this.minColor)) {
        const c = (/rgba?\((.*?)\)/.exec(this.minColor) || [
          "",
          "0,0,0,1",
        ])[1].split(",");
        color2.r = Number(c[0]) || 0;
        color2.g = Number(c[1]) || 0;
        color2.b = Number(c[2]) || 0;
      }

      ratio = parseFloat(ratio).toFixed(2);
      const r = Math.ceil(color1.r * ratio + color2.r * (1 - ratio));
      const g = Math.ceil(color1.g * ratio + color2.g * (1 - ratio));
      const b = Math.ceil(color1.b * ratio + color2.b * (1 - ratio));
      return `rgb(${r},${g},${b})`;
    },
    createHandlers(country) {
      return {
        mouseenter: (e) => this.enter(country, e),
        mouseleave: () => this.leave(),
        touchstart: (e) => this.enter(country, e),
        touchend: () => this.leave(),
      };
    },
    enter(country, e) {
    ...
    bindEvent() {
      if (this.$refs.country) {
        this.$refs.country.forEach((el) => {
          let events = this.createHandlers(el.getAttribute("data-id"));
