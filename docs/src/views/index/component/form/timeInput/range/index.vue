
<template>
  <section>
    <r-meta title="rTimeRangeInput Component | Renusify" description="Document with example for Renusify rTimeRangeInput component."></r-meta>
    <div class="mb-2">
      <h1 class="display-3 font-weight-light py-1 br-lg">
       rTimeRangeInput Component
      </h1>
    </div>
    <table-props :items="info.props"></table-props>
    <table-events :items="info.emits"></table-events>
    <table-provide :items="info.provide"></table-provide>
    <table-expose :items="info.expose"></table-expose>
    <table-slots :items="info.slots"></table-slots>
  </section>
  <br/>
  <section v-for="(item,i) in info.example" :key="i">
    <div class="title-1">{{i}}</div>
    <p class="caption">Press run Button</p>
    <r-code-editor
        runnable
        scriptSetup
        :script='item.script'
        :template='item.template'
        :css='item.style'
    >
    </r-code-editor>
  </section>
</template>
<script setup>
const info={
  "example": {
    "basic usage": {
      "template": "<rTimeRangeInput \n :withSec=\"withSec\"\n :noOverlay=\"noOverlay\"\n v-model=\"modelValue\"\n@update:modelValue=\"log('update:modelValue',$event)\">\n</rTimeRangeInput>",
      "script": "import {ref} from 'vue';\n\nconst withSec=ref(null)\nconst noOverlay=ref(null)\nconst modelValue=ref(() => [])\nconst log=(name,e)=>{\n        console.log(name,e)\n        }"
    }
  },
  "props": [
    {
      "name": "withSec",
      "type": "Boolean",
      "default": "undefined"
    },
    {
      "name": "noOverlay",
      "type": "Boolean",
      "default": "undefined"
    },
    {
      "name": "modelValue",
      "type": "Array",
      "default": "() => []"
    }
  ],
  "emits": [
    {
      "name": "update:modelValue"
    }
  ],
  "slots": [],
  "expose": [],
  "provide": []
}
</script>